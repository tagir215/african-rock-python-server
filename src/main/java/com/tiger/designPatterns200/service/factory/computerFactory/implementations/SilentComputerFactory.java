...
public class SilentComputerFactory extends AbstractComputerFactory{
    ...
    public SilentComputerFactory(ComponentSelector selector) {
    	super(selector);
    	info = new ComputerInfo();
		info.setX_case(0.05f);
		info.setX_cooler(0.2f);
		info.setX_cpu(0.2f);
		info.setX_gpu(0.05f);
		info.setX_motherboard(0.1f);
		info.setX_powerUnit(0.15f);
		info.setX_ram(0.1f);
		info.setX_storage(0.15f);
	}
